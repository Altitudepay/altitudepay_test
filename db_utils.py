...
def run_processor_query():
    query = """
        ...
        LEFT JOIN public.altitude_project p ON t.project_id = p.project_id
        LEFT JOIN public.altitude_customers c ON t.txid = c.txid
        WHERE EXTRACT(YEAR FROM t.created_date) = EXTRACT(YEAR FROM CURRENT_DATE - INTERVAL '2 month')
          AND EXTRACT(MONTH FROM t.created_date) = EXTRACT(MONTH FROM CURRENT_DATE - INTERVAL '2 month')
    """
    try:
        conn = psycopg2.connect(
            host=os.getenv("DB_HOST"),
