...
# Remove bins with very low success rates
...
# Add bin_seen_flag
# ---------------------------
seen_bins_file = "stats/processor_success_stats_latest.pkl"
if os.path.exists(seen_bins_file):
    with open(seen_bins_file, "rb") as f:
        past_stats = pickle.load(f)
    seen_bins = set(past_stats["bin_tx"].keys())
    df['bin_seen_flag'] = df['bin'].apply(lambda x: 1 if str(x) in seen_bins else 0)
else:
    df['bin_seen_flag'] = 1  # Assume all are seen if stats are missing
...
# ---------------------------
# Load Existing Model and Booster
# ---------------------------
...
y_holdout_prob_prev = prev_model.predict_proba(X_holdout)[:, 1]
old_acc = accuracy_score(y_holdout, y_holdout_pred_prev)

# ---------------------------
# Train New Model Incrementally
# ---------------------------
...
print("[RESULT] Previous Model Accuracy:", round(old_acc * 100, 2), "%")
print("[RESULT] New Data Accuracy:", round(new_model_accuracy * 100, 2), "%")
print("[RESULT] AUC:", roc_auc_score(y_holdout, y_prob))
print("\n[REPORT]\n", classification_report(y_holdout, y_pred, zero_division=0))
...
if new_model_accuracy >= 0.87:
    ...
    print("[SUCCESS] Model updated (accuracy >= 87%)")
else:
    print(f"[WARNING] Model NOT updated (accuracy {new_model_accuracy * 100:.2f}%) - below 87% threshold")
